function [net, info] = MSNet_train(net, spd_train, opts)
...
for epoch=1:opts.numEpochs 
    ...
    learningRate = 0.8^(count) * learningRate;
    if learningRate <=  opts.learningRate(epoch)/10
        learningRate = opts.learningRate(epoch)/10;
    end
     % fast-forward to last checkpoint
     modelPath = @(ep) fullfile(opts.dataDir, '/model',sprintf('net-epoch-%d.mat', ep));
     modelFigPath = fullfile(opts.dataDir, '/model','net-train.pdf') ;
     if opts.continue 
    ...
    train = opts.train(randperm(length(opts.train))) ; 
    val = opts.val;
...
for ib = 1 : batchSize : length(trainInd) 
    ...
    for ii = 1:length(net.layers)
        if strcmp(net.layers{ii}.type,'select')
            l1_loss = norm(net.layers{ii}.weight,1) * net.layers{ii}.lambda;
            break;
        else
            l1_loss = 0;
        end
...
for l=numel(net.layers):-1:1
  if isempty(res(l).dzdw)==0
    ...
    if isfield(net.layers{l}, 'weight')
        if (strcmp(net.layers{l}.type,'bfc')==1) || (strcmp(net.layers{l}.type,'bfc2')==1)
